...
#### 選定の観点
- 埋め込み元のデータ：モデルごとに対応しているデータフォーマットが異なる（テキスト、画像、マルチモーダルなど）
- 精度（ベンチマーク）: 目的のタスクに最適なベンチマークで比較します。
- 次元：モデルが提供するEmbeddingの次元が異なっています。一般的に次元が高いと精度が高まりますが、計算コストが大きくなります。
- レイテンシー: モデルによって、レスポンスのパフォーマンスが異なります。
- 言語: モデルによって対応している言語が異なります。
- カットオフ: LLMと同様にカットオフ（トレーニングデータに含まれるデータ期間）があります。
- 価格：トークン数ベースの課金になりますが、モデルごとに単価が異なります。

#### ベンチマーク
ベンチマークの指標も様々ありますが、以下に代表的なものをあげます。
...
#### Azure での選択肢
テキスト：
- Azure OpenAI でいくつかの Embedding Model を提供しています。text-embedding-3-small は、レイテンシーとストレージに最適化されています。一方、text-embedding-3-large は精度を高めるための優れたオプションであり、新しい dimensions パラメーターを利用して、全体的なパフォーマンスに影響を与えることなく、ネイティブ サイズの 3072 ではなく 1536 に維持することもできます。
- Azure AI Model Catalog で Cohere などの Embedding Model を提供しています。

画像：
- Azure AI Vision でマルチモーダル（テキストと画像）に対応した Embedding Model を提供しています。
